...
public function getStravaId()
    {
		$conn = getConnection();
		$sql = "SELECT s_client_id FROM tl_runners WHERE tl_runners.id = '{$this->getMemberId()}'";
		$stmt = $conn->query($sql);
    ...
    public function getMemberName()
    {
        ...
        {
            $conn = getConnection();
            $sql = "SELECT member_name FROM tl_runners WHERE tl_runners.id = ?";
    ...
    public function login($memberEmail, $memberPassword, $rememberMe)
    {
        ...
        $sql = "SELECT * FROM tl_runners WHERE tl_runners.email = ? AND tl_runners.member_num = ?";
        $stmt = $conn->prepare($sql);
        $stmt->execute(array($memberEmail, $memberPassword));
        $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
        if (empty($result))
        {
