...
$sql = "SELECT (SUM(run_distance) + SUM(warmup_distance) + SUM(cooldown_distance)) as weekly, count(*) as a FROM  tl_events WHERE (runner_id = :runner_id AND date(run_date) >= :week_start AND date(run_date) <= :week_end AND run_type_id != '9' AND run_type_id != '10' AND run_type_id != '11')";
