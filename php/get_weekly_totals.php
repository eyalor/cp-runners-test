...
try {
	...
	foreach ($weekends as $weekend) {
		...
		$sql = "SELECT (SUM(run_distance) + SUM(warmup_distance) + SUM(cooldown_distance)) as weekly FROM  tl_events WHERE (runner_id = :runner_id AND date(run_date) >= :week_start AND date(run_date) <= :week_end)";
