...
"SELECT tl_events.run_date ,tl_events.id, tl_runners.member_name AS 'name', tl_run_types.type, run_distance, run_time, warmup_distance, cooldown_distance, COALESCE(tl_events.notes, '') AS 'notes', tl_events.run_type_id
    FROM tl_events
	...
	and tl_events.run_date <= '" . $untilDate . "'
...
"SELECT events.id AS event_id, comments.id AS comment_id, comments.runner_id, runners.member_name as 'commenter_name', COALESCE(comments.comment, '') AS comment
     ...
     WHERE events.run_date > '" . $date . "' and events.run_date <= '" . $untilDate . "'  and comments.runner_id IN ('" . $runnerId . "', 122, 124)
