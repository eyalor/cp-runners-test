...
<script type="text/javascript">
	...
	function getMyNextDayEvents()
    {
        myDate.setDate(myDate.getDate() -30);
        feedQueue.push(new Date(myDate));
        showMyComments();
    }

    function onMyFeedScroll()
    {
        if (feedQueueLock) {
            return;
        }
	...
	function showMyComments(){
        ...
        feedQueueLock = false;
        date = new Date()-350;
        myFeedQueue.push(new Date(date));
        ...
        $.ajax({
            ...
            success: function (txt) {
                ...
                if (doc.status.ecode == STATUS_ERR) {
                    ...
                    feed.append('<div class="day_header">'+ $memberAuthentication->getMemberId()+'</div>');
                    var $dayEvents = $('<div class="day_events"></div>');
                    feed.append($dayEvents);
                    if (doc.data['events'].length == 0) {
                        $dayEvents.append('<div class="event">אין נתונים ליום זה</div>');
...
<div class="content">
    <h2 id="team_events_header" class="page_header">מה עשו החבר'הddd</h2>
    <a id="show_new_comments_action" href="#" onclick="showNewComments(); return false;" style="display:none;">הצג פירגונים חדשים</a>
	<a id="show_my_comments_action" href="#" onclick="showMyComments(); return false;" >הצג הערות מאמנים</a>
