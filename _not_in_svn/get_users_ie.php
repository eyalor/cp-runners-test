...
try {
	...
	if(isset($_GET['search_term'])) {
		...
		$sqlPhrase = "SELECT distinct tl_runners.id,tl_runners.member_name FROM tl_runners,tl_events WHERE run_date > '" . date('Y-m-d', $the_date) . "' and tl_runners.id=tl_events.runner_id order by tl_runners.member_name"; 
	}
...
function getUsersAsJSON($conn,$sqlPhrase) {
	...
	return returnJSONsuccess($result[0]);
}
